...
<div class="space-y-5">
  <div style="display:flex; align-items:center; gap:1rem;">
    <span onclick={() => navigate('#/apps')} style="cursor:pointer; color:#8b8579; font-size:0.85rem;">← Apps</span>
    ...
    {#if detail?.app?.trust_tier}
      <span style="padding:0.15rem 0.5rem; border-radius:10px; font-size:0.75rem; background:rgba(74,111,168,0.2); color:#4a6fa8;">{detail.app.trust_tier}</span>
    {/if}
  </div>
  {#if error}
  ...
  {#if detail}
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <DashboardCard accent="green">
        <div style="font-size:0.75rem; color:#8b8579; text-transform:uppercase;">Requests Today</div>
        <div style="font-size:1.5rem; font-weight:700; color:#2d2a26;">{detail.stats?.today?.requests || 0}</div>
      </DashboardCard>
      <DashboardCard accent="blue">
        <div style="font-size:0.75rem; color:#8b8579; text-transform:uppercase;">Cost Today</div>
        <div style="font-size:1.5rem; font-weight:700; color:#2d2a26;">${(detail.stats?.today?.total_cost || 0).toFixed(4)}</div>
      </DashboardCard>
      <DashboardCard accent="orange">
        <div style="font-size:0.75rem; color:#8b8579; text-transform:uppercase;">Avg Latency</div>
        <div style="font-size:1.5rem; font-weight:700; color:#2d2a26;">{Math.round(detail.stats?.today?.avg_latency || 0)}ms</div>
      </DashboardCard>
      <DashboardCard accent="green">
        <div style="font-size:0.75rem; color:#8b8579; text-transform:uppercase;">Cost This Month</div>
        <div style="font-size:1.5rem; font-weight:700; color:#2d2a26;">${(detail.stats?.month?.total_cost || 0).toFixed(2)}</div>
      </DashboardCard>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <DashboardCard title="Usage Over Time" accent="blue">
        ...
        {:else}
          <div style="text-align:center; color:#8b8579; padding:2rem;">No usage data yet</div>
        {/if}
      </DashboardCard>
      <DashboardCard title="Backend Mix" accent="green">
        ...
        {:else}
          <div style="text-align:center; color:#8b8579; padding:2rem;">No provider data yet</div>
        {/if}
      </DashboardCard>
    </div>
    <DashboardCard title="API Keys" accent="blue">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
        <span style="font-size:0.78rem; color:#8b8579;">{keys.filter(k => !k.revoked_at).length} active keys</span>
      ...
      {#if newKeyShown}
        <div style="background:rgba(39,134,74,0.08); border:1px solid rgba(39,134,74,0.2); border-radius:6px; padding:0.75rem; margin-bottom:1rem;">
          <div style="font-size:0.75rem; color:#27864a; margin-bottom:0.3rem;">New key (copy now — you won't see it again)</div>
          <code style="font-size:0.78rem; color:#2d2a26; word-break:break-all;">{newKeyShown}</code>
        </div>
      {/if}
      {#each keys.filter(k => !k.revoked_at) as key}
        <div style="display:flex; align-items:center; justify-content:space-between; padding:0.5rem; background:#e8e2d8; border-radius:6px; margin-bottom:0.5rem;">
          <div>
            <code style="font-size:0.78rem; color:#2d2a26;">{key.key_prefix}...</code>
            <span style="margin-left:0.5rem; font-size:0.75rem; color:#8b8579;">
              {key.last_used_at ? `used ${new Date(key.last_used_at * 1000).toLocaleDateString()}` : 'never used'}
            </span>
          </div>
          <div style="display:flex; gap:0.5rem;">
            <button onclick={() => rotateKey(key.id)} style="font-size:0.75rem; background:none; border:none; cursor:pointer; color:#4a6fa8;">rotate</button>
            <button onclick={() => revokeKey(key.id)} style="font-size:0.75rem; background:none; border:none; cursor:pointer; color:#c0392b;">revoke</button>
          </div>
        </div>
      {:else}
        <div style="text-align:center; color:#8b8579; font-size:0.78rem; padding:1rem;">No active keys</div>
      {/each}
    </DashboardCard>
    <DashboardCard title="Routing Policy" accent="orange">
      <form onsubmit={savePolicy} style="display:flex; flex-direction:column; gap:1rem;">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label style="display:block; font-size:0.75rem; color:#8b8579; margin-bottom:0.3rem;">Allowed providers (comma-separated)</label>
            <input bind:value={policyForm.allowed_providers} placeholder="openai, anthropic" style={inputStyle} />
          </div>
          <div>
            <label style="display:block; font-size:0.75rem; color:#8b8579; margin-bottom:0.3rem;">Blocked providers (comma-separated)</label>
            <input bind:value={policyForm.blocked_providers} placeholder="cerebras" style={inputStyle} />
          </div>
          <div>
            <label style="display:block; font-size:0.75rem; color:#8b8579; margin-bottom:0.3rem;">Data residency</label>
          ...
          <div>
            <label style="display:block; font-size:0.75rem; color:#8b8579; margin-bottom:0.3rem;">Max cost per request ($)</label>
            <input type="number" step="0.001" bind:value={policyForm.max_cost_per_request} placeholder="0.10" style={inputStyle} />
          </div>
          <div>
            <label style="display:block; font-size:0.75rem; color:#8b8579; margin-bottom:0.3rem;">Rate limit (RPM)</label>
            <input type="number" bind:value={policyForm.rate_limit_rpm} placeholder="60" style={inputStyle} />
          </div>
          <div>
            <label style="display:block; font-size:0.75rem; color:#8b8579; margin-bottom:0.3rem;">Token limit (daily)</label>
        ...
        <div style="display:flex; align-items:center; gap:0.75rem;">
          ...
          <label for="force_local_pii" style="font-size:0.85rem; color:#2d2a26;">Force local providers when PII detected</label>
        </div>
        {#if policySaved}
  ...
  {:else if !error}
    <div style="color:#8b8579; font-size:0.85rem;">loading app detail...</div>
  {/if}
</div>
